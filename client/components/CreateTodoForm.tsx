...
function CreateTodoForm() {
  ...
  return (
    <form className={'w-full p-5 bg-slate-100'} onSubmit={handleSubmit}>
      <input className={'block w-full h-12 px-3 rounded-lg'} value={title} onChange={onChangeTitle} placeholder="해야할 일을 메모해볼까요?"/>
      <textarea className={'block w-full mt-3 p-3 rounded-lg'} value={content} onChange={onChangeContent} placeholder="자세히 작성해 봐요"/>
